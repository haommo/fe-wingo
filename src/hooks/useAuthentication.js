...
export function useAuthentication() {
  ...
  const isRoleAdmin = userCurrentInfo?.role === 'RL1' || userCurrentInfo?.role === 'RL2';

  const {
    mutate: loginUser,
    error: loginError,
    isLoading: loadingLogin
  } = useMutation(async (payload) => await authApi.login(payload), {
  ...
  return { loginUser, isLoggedIn, logoutUser, loginError, userCurrentInfo, isRoleAdmin, loadingLogin };
}
