...
const PackageItem = ({ onDeleteItem, index, control }) => {
  ...
  return (
    <>
      ...
      <TableCell sx={{ '& .MuiFormHelperText-root': { position: 'absolute', bottom: -24, ml: 0 }, minWidth: '130px' }}>
        <InputField control={control} name={`packageInfo.${index}.length`} type="number" placeholder="length (cm)" />
      </TableCell>

      <TableCell sx={{ '& .MuiFormHelperText-root': { position: 'absolute', bottom: -24, ml: 0 }, minWidth: '130px' }}>
        <InputField control={control} name={`packageInfo.${index}.width`} type="number" placeholder="width (cm)" />
      </TableCell>

      <TableCell sx={{ '& .MuiFormHelperText-root': { position: 'absolute', bottom: -24, ml: 0 }, minWidth: '130px' }}>
        <InputField control={control} name={`packageInfo.${index}.height`} type="number" placeholder="height (cm)" />
      </TableCell>

      <TableCell sx={{ '& .MuiFormHelperText-root': { position: 'absolute', bottom: -24, ml: 0 }, minWidth: '130px' }}>
        <InputField control={control} name={`packageInfo.${index}.weight`} type="number" placeholder="weight (kg)" />
      </TableCell>
