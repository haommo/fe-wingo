...
export default function AddUserDialog({ open, setOpen, mode, data, setMode, onRemove, isEditProfile }) {
  ...
  useEffect(() => {
    if ((data && mode.edit && open) || isEditProfile) {
      clearErrors();
  ...
  }, [countryValue, setValue, data, mode.edit, mode.add, open, clearErrors, isEditProfile]);
  ...
  const onSubmit = (formValue) => {
    ...
    payload.append('number', formValue?.userForm[0]?.number);
    ...
    payload.append(
      'address',
      mode.edit
        ? formValue.userForm[0]?.address_first
        : isEditProfile
        ? formValue.userForm[0]?.address_first
        : getAddressFromGGmaps(formValue.userForm[0]?.address_first)
    );
