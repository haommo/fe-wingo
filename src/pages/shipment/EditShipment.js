...
const schemaForm1 = yup
  .object({
    ...
    // created_for_rl1: yup.object().required(),
...
export default function EditShipment() {
  ...
  const { mutate, isLoading: loadingEdit } = useUpdateShipment();
  const { userCurrentInfo } = useAuthentication();
  ...
  const onConfirm = (lastChildData) => {
    const serviceAddress = {
      ...
      company: formValues?.generalInfo?.senderInfo[0]?.company
        ? formValues?.generalInfo?.senderInfo[0]?.company
        : formValues?.generalInfo?.senderInfo[0]?.name
    };
    ...
    const receiverAddress = {
      ...
      company: formValues?.generalInfo?.receiverInfo[0]?.company
        ? formValues?.generalInfo?.receiverInfo[0]?.company
        : formValues?.generalInfo?.receiverInfo[0]?.name
    };
    ...
    payload.append(
      'reason_export',
      formValues.generalInfo.reason_export
        ? formValues.generalInfo.reason_export
        : formValues.generalInfo.shipment_content
        ? formValues.generalInfo.shipment_content
        : 'none'
    );
    ...
    payload.append('status', formValues.generalInfo?.status);
    payload.append('localtracking', formValues.generalInfo?.localtracking);
    payload.append('localcode', formValues.generalInfo?.localcode);

    payload.append(
      'created_for',
      formValues.generalInfo?.created_for_rl1?.value ? formValues.generalInfo?.created_for_rl1?.value : userCurrentInfo.uuid
    );
            ...
            <Grid item xs={12}>
              <TabPanel value={step} index={0}>
                <SenderAndReceiverInfomation
                  ...
                  watch={watchForm1}
                  edit={true}
                  manifestId={shipmentData?.Manifest_id}
                />
              </TabPanel>
