...
export default function EditManifest() {
  ...
  const total = watch('list_shipments')?.length?.toString();

  useEffect(() => {
    if (id) {
      setValue('date', dayjs(data?.manifest_Shipment?.date));
    ...
    <>
      <form onSubmit={handleSubmit(onConfirm)}>
        <MainCard boxShadow={true}>
          ...
          <ListManifestHistory control={control} isLoading={isLoading} />
        </MainCard>
      </form>
    </>
  );
